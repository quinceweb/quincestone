...
create or replace function public.commerce_publish_product(target_product uuid) returns boolean language plpgsql security invoker set search_path='' as $$ begin if not private.is_platform_admin((select auth.uid())) then raise exception 'platform administrator authority required'; end if; if not private.commerce_product_launch_ready(target_product) then raise exception 'product launch gates are incomplete'; end if; update public.commerce_products set lifecycle_status='active',merchandising_status='published',publication_state='published' where id=target_product; insert into public.commerce_audit_events(actor_user_id,action,resource_type,resource_id,metadata) values((select auth.uid()),'product.published','product',target_product::text,jsonb_build_object('launch_gate','passed')); return true; end; $$;
