...
create index if not exists commerce_shopify_products_sync_idx on public.commerce_shopify_products(sync_status, updated_at desc);
create index if not exists commerce_shopify_variants_sync_idx on public.commerce_shopify_variants(sync_status, updated_at desc);
...
create policy commerce_shopify_products_operator on public.commerce_shopify_products for all to authenticated using (private.is_platform_admin((select auth.uid()))) with check (private.is_platform_admin((select auth.uid())));

drop policy if exists commerce_shopify_variants_operator on public.commerce_shopify_variants;
create policy commerce_shopify_variants_operator on public.commerce_shopify_variants for all to authenticated using (private.is_platform_admin((select auth.uid()))) with check (private.is_platform_admin((select auth.uid())));
