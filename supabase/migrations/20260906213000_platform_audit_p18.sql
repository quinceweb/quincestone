...
create or replace function private.record_platform_audit(
  target_actor uuid,
  target_action text,
...
begin
  if auth.uid() is null or auth.uid() <> target_actor then
    raise exception 'trusted actor identity required';
  end if;
