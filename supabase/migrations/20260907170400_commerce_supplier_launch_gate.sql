create or replace function private.commerce_product_launch_ready(target_product uuid) returns boolean language sql stable security definer set search_path='' as $$
select exists(
  select 1
  ...
  where p.id=target_product
    ...
    and exists(select 1 from public.commerce_supplier_products sp join public.commerce_variants v on v.id=sp.variant_id join public.commerce_suppliers s on s.id=sp.supplier_id where v.product_id=p.id and sp.is_selected and s.status='approved')
