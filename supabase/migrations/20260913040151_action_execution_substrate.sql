...
create index if not exists action_executions_workspace_status_idx on public.action_executions (workspace_id, status, created_at desc);
create index if not exists action_executions_interaction_idx on public.action_executions (interaction_id, created_at desc);
create index if not exists action_executions_review_idx on public.action_executions (human_review_id);
create index if not exists action_executions_provider_reference_idx on public.action_executions (provider, provider_reference) where provider_reference is not null;
...
create policy action_executions_select_member on public.action_executions for select to authenticated using ((select private.is_workspace_member(workspace_id)));
...
create trigger action_executions_updated_at before update on public.action_executions for each row execute function private.touch_action_executions_updated_at();
...
begin
  ...
  if should_execute then
    ...
    action_hash := md5(proposed_action::text);
