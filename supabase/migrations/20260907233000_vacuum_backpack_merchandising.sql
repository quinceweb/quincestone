...
alter table public.commerce_product_media add column if not exists verification_status text not null default 'unverified' check (verification_status in ('unverified','verified'));
alter table public.commerce_product_media add column if not exists publication_status text not null default 'unpublished' check (publication_status in ('unpublished','published','restricted'));
alter table public.commerce_product_media add column if not exists sku_association text;
alter table public.commerce_product_media add column if not exists aspect_ratio numeric;
alter table public.commerce_product_media add column if not exists width integer;
alter table public.commerce_product_media add column if not exists height integer;
alter table public.commerce_product_media add column if not exists storage_path text;
alter table public.commerce_product_media add column if not exists notes text;
...
create index if not exists commerce_media_publication_idx on public.commerce_product_media(product_id, publication_status, verification_status, sort_order);
create index if not exists commerce_product_content_updated_idx on public.commerce_product_content(updated_at desc);
...
create policy commerce_product_content_public on public.commerce_product_content for select to anon, authenticated using (exists(select 1 from public.commerce_products p where p.id=product_id and p.lifecycle_status='active' and p.merchandising_status='published' and p.publication_state='published'));

-- Recreate rather than replace because the existing view's column order is part of
-- its public contract and PostgreSQL does not allow CREATE OR REPLACE to insert columns.
...
select
  ...
  m.id as media_id,m.asset_url,m.media_type,m.alt_text,m.sort_order,m.source,m.verification_status,m.publication_status,m.aspect_ratio,m.width,m.height
...
select exists(select 1 from public.commerce_products p join public.commerce_product_qa q on q.product_id=p.id where p.id=target_product and q.sourcing_status='verified' and q.exact_sku_verified and q.sample_status='approved' and q.quality_result='pass' and q.functional_result='pass' and q.packaging_result='pass' and q.media_verification='approved' and q.economics_approval and q.shipping_terms_approval and q.returns_warranty_approval and q.price_approval and q.content_approval and q.final_decision='ready' and exists(select 1 from public.commerce_variants v where v.product_id=p.id and v.status='active' and v.price_amount is not null) and not exists(select 1 from public.commerce_product_media m where m.product_id=p.id and (m.rights_status<>'approved' or not m.publication_permission or m.verification_status<>'verified' or m.publication_status<>'published')));
$$;
