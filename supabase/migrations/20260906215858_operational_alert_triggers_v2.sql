...
as $$
begin
  perform private.emit_operational_alert(new.workspace_id, 'integration', 'provider_execution_failure', 'medium', 'Integration execution failed', 'A provider integration event entered a failed state.', 'integration:' || coalesce(new.provider, 'unknown') || ':' || coalesce(new.event_type, 'unknown') || ':failure', coalesce(new.request_id, new.source_record_id::text, new.id::text), jsonb_build_object('provider', new.provider, 'event_type', new.event_type, 'status', new.status, 'error_code', new.error_code, 'request_id', new.request_id));
  return new;
end;
$$;
