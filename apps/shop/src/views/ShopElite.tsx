...
export function ShopEliteProduct({ slug }: { slug: string }) {
  ...
  const variant = product.variants.find((item) => item.variant_id === variantId) || product.variants[0];
  ...
  return <div className="qs-elite-page qs-elite-product"><div className="qs-elite-product-hero"><div className="qs-elite-gallery"><div className="qs-elite-gallery-stage">{image?.asset_url ? <img src={image.asset_url} alt={image.alt_text || product.name} /> : <div className="qs-elite-media-pending">VERIFIED MEDIA NOT AVAILABLE</div>}</div>{product.media.length > 1 && <div className="qs-elite-thumbs">{product.media.map((m, i) => <button key={m.media_id} type="button" aria-label={`View image ${i + 1}`} aria-pressed={active === i} onClick={() => setActive(i)}><img src={m.asset_url || ""} alt="" /></button>)}</div>}</div><aside className="qs-elite-purchase"><p className="eyebrow">{product.content_eyebrow || product.collection.toUpperCase()}</p><h1>{product.content_headline || product.name}</h1><p>{product.content_subheadline || product.content_short_description || product.description}</p>{variant?.price_amount ? <strong className="qs-elite-price">{money(variant.price_amount, variant.currency)}</strong> : <span>PRICE NOT AVAILABLE</span>}{product.variants.length > 1 && <label className="qs-elite-variant"><span>Variant</span><select value={variant?.variant_id || ""} onChange={(event) => setVariantId(event.target.value)}>{product.variants.map((item) => <option key={item.variant_id} value={item.variant_id}>{Object.values(item.option_values || {}).filter(Boolean).join(" / ") || item.sku}</option>)}</select></label>}<div className="qs-elite-availability"><span>PUBLICATION</span><strong>{variant ? "Published product record" : "No sellable variant"}</strong><small>Checkout revalidates availability, price and order state server-side.</small></div><button className="button" type="button" disabled={!variant || variant.price_amount == null} onClick={() => { if (!variant) return; const raw = localStorage.getItem("qs-commerce-cart") || "[]"; let cart: { variantId: string; quantity: number }[] = []; try { cart = JSON.parse(raw); } catch { cart = []; } const found = cart.find((x) => x.variantId === variant.variant_id); const next = found ? cart.map((x) => x.variantId === variant.variant_id ? { ...x, quantity: x.quantity + 1 } : x) : [...cart, { variantId: variant.variant_id, quantity: 1 }]; localStorage.setItem("qs-commerce-cart", JSON.stringify(next)); window.dispatchEvent(new Event("qs-cart-change")); }}>{variant?.price_amount == null ? "Purchase unavailable" : "Add to bag"}</button><Link className="text-link" to="/bag">View bag →</Link></aside></div><section className="qs-elite-standard"><div><p className="eyebrow">THE QUINCESTONE STANDARD</p><h2>Why Quincestone chose this.</h2><p>The product is visible because the Commerce OS authorized publication. The public record below distinguishes disclosed evidence from internal controls.</p></div><div>{standards.map(([name, body]) => <details key={name}><summary><span>{name}</span><b>Publication gate</b></summary><p>{body} Supporting supplier identities, private economics and restricted evidence remain behind the operating boundary.</p></details>)}</div></section><section className="qs-elite-two-col"><article><p className="eyebrow">WHAT WE LIKE</p><h2>Evidence-backed strengths.</h2>{benefits.length ? <ul>{benefits.map((x,i)=><li key={i}>{value(x.title) || value(x.text) || value(x.description)}</li>)}</ul> : <p>Published product content does not currently include additional strengths.</p>}</article><article><p className="eyebrow">WHAT THE PUBLIC RECORD SUPPORTS</p><h2>Structured product detail.</h2>{features.length ? <ul>{features.map((x,i)=><li key={i}>{value(x.title)}{value(x.text) ? ` — ${value(x.text)}` : ""}</li>)}</ul> : <p>Additional product detail will appear only when it is present in the published record.</p>}</article></section>{(specs.length || included.length) ? <section className="qs-elite-specs"><p className="eyebrow">DETAILS</p><div>{[...specs, ...included].map((x,i)=><div key={i}><span>{value(x.title) || value(x.name) || `Detail ${i + 1}`}</span><strong>{value(x.value) || value(x.text) || value(x.description)}</strong></div>)}</div></section> : null}<section className="qs-elite-product-note"><p className="eyebrow">WHAT YOU SHOULD KNOW</p><p>{product.content_story || "Product context is presented from the published record. Unsupported claims are deliberately omitted."}</p></section></div>;
}
