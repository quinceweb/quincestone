...
export default async function InteractionsPage() {
  ...
  const { data: interactions, error } = await supabase
    .from("interactions")
    .select("id, source, status, intent, created_at, customer_id")
  ...
  return (
    <section className="page-section">
      ...
      <h1>Interactions</h1>
      <p className="lede">Customer demand enters Quincestone as an interaction. This surface becomes the operational record of what arrived, what Edge understood and what happened next.</p>
          ...
          {interactions!.map((interaction) => (
            <article className="record" key={interaction.id}>
              <div>
                ...
                <p className="empty">{new Date(interaction.created_at).toLocaleString()} · {interaction.customer_id ? "Known customer" : "No customer linked"}</p>
              </div>
