...
const steps: Step[] = [
  ...
  { id: "demand", eyebrow: "06 / DEMAND", title: "Where do opportunities usually begin?", prompt: "Choose the strongest sources of customer intent.", options: ["Search / organic traffic", "Advertising / paid traffic", "Referrals / word of mouth", "Social / content", "Existing customers / repeat business", "Several of these are important"] },
...
export function EdgeAssessment() {
  ...
  async function finish() {
    ...
    const report = {
      ...
      score: result.score,
      flags: result.flags,
      priority: result.score >= 55 ? "high" : result.score >= 30 ? "medium" : "foundational",
      recommendation: result.score >= 55
        ? "Prioritize the interaction-to-outcome path: qualification, knowledge, policy and routing."
        : result.score >= 30
          ? "Map the customer journey and introduce a structured Edge front door around the highest-value gap."
          : "Establish the operating model first, then design the simplest digital front door around it.",
    };
    const payload = {
      name: answers.name ?? "",
      email: answers.email ?? "",
      ...
      message: JSON.stringify({ version: 1, answers, report }),
      company_site: "",
      assessment: JSON.stringify({ answers, report }),
    };
