...
export default async function CommerceProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: product }, { data: qa }, { data: content }, { data: variants }, { data: media }, { data: supplierLinks }] = await Promise.all([
    supabase.from("commerce_products").select("*").eq("id", id).maybeSingle(),
    ...
    supabase.from("commerce_supplier_products").select("id,supplier_id,supplier_sku,supplier_product_id,quoted_unit_cost,currency,moq,quote_date,media_rights_state,is_selected").in("variant_id", (await supabase.from("commerce_variants").select("id").eq("product_id", id)).data?.map((v) => v.id) || []),
  ]);
  if (!product) notFound();
  ...
  return <main className="main"><div className="content">
    ...
    <section className="section commerce-table"><div className="section-heading"><div><div className="eyebrow">MEDIA REVIEW</div><h2>Rights and SKU evidence</h2></div><span>{media?.length ?? 0} assets</span></div>{!media?.length ? <p>No media is attached. This is safe but keeps media verification blocked.</p> : <div className="commerce-product-list">{media.map((asset) => <article className="commerce-product-row" key={asset.id}><div><strong>{asset.media_type.toUpperCase()}</strong><p>{asset.asset_url}</p><small>{status(asset.source)} · {status(asset.verification_status)} · {status(asset.publication_status)}</small></div><div className="media-actions"><form action={async () => { "use server"; await reviewProductMedia(id, asset.id, "APPROVE"); }}><button type="submit">Approve</button></form><form action={async () => { "use server"; await reviewProductMedia(id, asset.id, "MARK_CONCEPT"); }}><button type="submit">Concept</button></form><form action={async () => { "use server"; await reviewProductMedia(id, asset.id, "MARK_VERIFIED"); }}><button type="submit">Verify</button></form><form action={async () => { "use server"; await reviewProductMedia(id, asset.id, "RESTRICT"); }}><button type="submit">Restrict</button></form><form action={async () => { "use server"; await reviewProductMedia(id, asset.id, "UNPUBLISH"); }}><button type="submit">Unpublish</button></form></div></article>)}</div>}</section>
