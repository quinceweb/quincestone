    ...
    <>
      <section className="hero hero-premium">
        <div className="hero-copy">
          ...
          <div className="actions"><Link className="button" to="/demo">Explore the system</Link><Link className="button secondary" to="/assessment">Request an Edge Assessment</Link></div>
      ...
      <section className="section statement">
        ...
        <div className="section-heading"><h2>Your website should not stop at information.</h2><p>Most digital experiences end when a visitor clicks, submits, or leaves. Quincestone is designed for what happens between those moments: interpreting intent, applying the organization's operating logic, and moving the interaction toward a useful outcome.</p></div>
      </section>

      <div className="capability-line">EDGE EXPERIENCE · AI CONCIERGE · QUALIFICATION INTELLIGENCE · BUSINESS KNOWLEDGE · WORKFLOW ROUTING · HUMAN ESCALATION</div>

      <section className="section">
        <p className="eyebrow">THE SYSTEM</p>
        <div className="section-heading"><h2>One operating layer. Seven connected capabilities.</h2><p>Designed as infrastructure rather than another marketing widget, Quincestone connects the experience layer to the operating reality behind the business.</p></div>
        <div className="grid capability-grid">{capabilities.map(([n, title, text]) => <article className="panel" key={n}><span>{n}</span><h3>{title}</h3><p>{text}</p></article>)}</div>
      </section>

      <section className="section dark-flow">
        <div className="section-heading"><div><p className="eyebrow">FROM VISITOR TO WORKFLOW</p><h2>Every interaction should produce a better next action.</h2></div><p>Quincestone combines deterministic rules, approved knowledge, bounded AI, and human review into a traceable journey.</p></div>
        <div className="flow-grid">{proof.map(([n, title, text]) => <article key={n}><span>{n}</span><h3>{title}</h3><p>{text}</p></article>)}</div>
      </section>

      <section className="section demo-promo">
        <div className="demo-promo-copy"><p className="eyebrow">INTERACTIVE DEMONSTRATION</p><h2>See a fictional request become an operational decision.</h2><p>Northstone Roofing is a fictional company created to demonstrate Quincestone. Walk through an emergency request, inspect every decision, then open the fictional operations desk.</p><div className="actions"><Link className="button" to="/demo/experience">Run the experience</Link><Link className="text-link" to="/demo/operations">Open operations →</Link></div></div>
        <div className="demo-frame"><div><span>VISITOR</span><strong>Intent detected</strong></div><div><span>INTELLIGENCE</span><strong>Policy applied</strong></div><div><span>WORKFLOW</span><strong>Human review</strong></div></div>
      </section>

      <section className="section offer">
        <p className="eyebrow">START WITH CLARITY</p>
        <div className="offer-grid"><div><h2>Edge Assessment</h2><p>A focused 30-minute assessment to identify where your website or application can better understand intent, qualify demand, and route work.</p></div><div className="offer-price"><span>ONE-TIME</span><strong>$49</strong><small>USD</small><Link className="button" to="/assessment">Request assessment</Link></div></div>
      </section>

      <section className="section midnight"><p className="eyebrow">QUINCEWEB / QUINCESTONE</p><h2>Build the layer between what your customers say and what your organization does.</h2><p>Start with one journey. Make the decision logic visible. Expand from there.</p><div className="actions"><Link className="button light" to="/assessment">Request an Edge Assessment</Link><Link className="button secondary light" to="/platform">Explore the platform</Link></div></section>
    </>
  );
}
