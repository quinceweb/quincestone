...
Both modes are served by `apps/web`, consume QVS 2.0 from `packages/config/src/brand.css`, and sit above the one canonical Quincestone Supabase backend. Shared genes do not require identical composition.
...
4. Modes — Corporate and Commerce compositions within `apps/web`; Account, Business, Deals and Admin remain separate application boundaries.
