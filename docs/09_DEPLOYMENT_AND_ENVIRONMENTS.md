...
| `quincestone-admin` | `apps/admin` | Next.js | no app `vercel.json` on inspected `main`; settings not exposed by connector | Vercel aliases only; no custom domain reported | **CANCELED**, exact inspected `main` SHA | None found in the 20 deployments inspected |
